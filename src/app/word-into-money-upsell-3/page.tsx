...
import { AlertCircle, Shield, Lock, Star, Calendar } from 'lucide-react';
...
function PaymentForm({ clientSecret, leadId, wimLeadId }: { clientSecret: string; leadId: string; wimLeadId: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error, paymentIntent } = await stripe.confirmPayment({
            elements,
            confirmParams: { return_url: `${window.location.origin}/word-into-money-final?leadId=${wimLeadId}` },
            redirect: 'if_required',
        });

        if (error) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (window as any).__track?.checkoutStep?.('payment_error', 'wim-upsell-3-checkout');
            setErrorMessage(error.message || 'An error occurred.');
            setIsProcessing(false);
        } else if (paymentIntent?.status === 'succeeded') {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (window as any).__track?.formSuccess?.('wim-upsell-3-checkout');
            await fetch('/api/word-into-money/upsell-3/confirm-payment', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ leadId, paymentIntentId: paymentIntent.id, wimLeadId }),
            });
            window.location.href = `/word-into-money-final?leadId=${wimLeadId}`;
        }
    };

    return (
        <form onSubmit={handleSubmit} data-track-form="wim-upsell-3-checkout">
            ...
            <button disabled={isProcessing || !stripe} className={`w-full mt-6 bg-brand-lime hover:bg-brand-limeDim text-brand-950 font-anton text-xl py-5 rounded-xl uppercase ${isProcessing ? 'opacity-70' : ''}`}>
                {isProcessing ? 'Processing...' : 'BOOK MY 1:1 COACHING — $997'}
            </button>
...
function Upsell3Content() {
    ...
    const startPayment = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const res = await fetch('/api/word-into-money/upsell-3/create-payment-intent', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (data.success) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (window as any).__track?.checkoutStep?.('payment_step', 'wim-upsell-3-checkout');
        ...
        <div className="min-h-screen bg-brand-950">
            <div className="max-w-[620px] mx-auto px-4 py-12">
                <div className="text-center mb-10">
                    <p className="text-brand-lime text-xs font-bold uppercase tracking-widest mb-3">Final Offer — VIP Only</p>
                    <h1 className="font-anton text-3xl md:text-5xl text-brand-white uppercase mb-4">1:1 Sales Coaching With Ana</h1>
                    <p className="font-lora text-lg text-brand-grey max-w-md mx-auto">
                        Want me to personally review your copy and build your 90-day monetisation plan? This is your chance.
                    </p>
                </div>

                <div data-track-section="offer" className="space-y-4 mb-10">
                    {[
                        { icon: Star, text: 'Private 60-minute strategy session with Ana' },
                        { icon: Calendar, text: 'Custom 90-day monetisation roadmap' },
                        { icon: Star, text: 'Personal copy review & feedback' },
                        { icon: Calendar, text: 'Priority email support for 30 days' },
                        { icon: Star, text: 'Recording of your session to revisit anytime' },
                    ].map((item, i) => (
                        <div key={i} className="flex items-center gap-3 bg-brand-900 border border-brand-800 rounded-xl p-4">
                            <item.icon size={18} className="text-brand-lime flex-shrink-0" />
                            <span className="text-brand-white text-sm font-lora">{item.text}</span>
                        </div>
                    ))}
                </div>
                ...
                <div data-track-section="checkout" className="bg-brand-900 border border-brand-800 rounded-2xl overflow-hidden">
                    ...
                    <div className="p-6 md:p-8">
                        {step === 'info' ? (
                            <form onSubmit={startPayment} data-track-form="wim-upsell-3-checkout" className="space-y-4">
                                ...
                                <button disabled={isLoading} data-track="cta" data-track-id="wim-upsell-3-accept" className="w-full bg-brand-lime hover:bg-brand-limeDim text-brand-950 font-anton text-xl py-5 rounded-xl uppercase">
                                    {isLoading ? 'Preparing...' : 'CONTINUE TO PAYMENT — $997'}
                                </button>
                            </form>
                ...
                <div className="text-center mt-6">
                    <a href={`/word-into-money-final?leadId=${wimLeadId}`} className="text-brand-grey text-sm hover:text-brand-white transition-colors">
