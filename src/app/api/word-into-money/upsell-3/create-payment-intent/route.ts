...
import { stripe, WIM_UPSELL3_PRICE } from '@/lib/stripe';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Create lead in wim_coaching_leads
        const { data: lead, error: supabaseError } = await supabaseAdmin
            .from('wim_coaching_leads')
            .insert({
                name,
                email,
                total_paid: WIM_UPSELL3_PRICE,
                is_paid: false,
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            ...
            metadata: {
                ...
                email,
                name,
                product: 'coaching_from_wim_upsell',
            },
        });
        ...
        console.error('Upsell 3 Stripe Exception:', error);
