...
function SuccessContent() {
    ...
    const [hasBump2, setHasBump2] = useState(false); // Hooks That Stop the Scroll
    const [hasBump3, setHasBump3] = useState(false); // Show Don't Tell
    ...
    useEffect(() => {
        ...
        if (leadId) {
            fetch(`/api/unstuck/get-lead-status?leadId=${leadId}`)
                .then(res => res.json())
                .then(data => {
                    if (data.success && data.lead.is_paid) {
                        ...
                        // Calculate total for FB Pixel
                        let total = 97;
                        if (data.lead.has_bundle) {
                            total += 69;
                        } else {
                            if (data.lead.has_bump3) total += 47; // SDT
                            if (data.lead.has_bump1) total += 27; // Genius
                            if (data.lead.has_bump2) total += 27; // Hooks
                        }
                        ...
                        if (typeof window !== 'undefined' && (window as any).fbq) {
                            (window as any).fbq('track', 'Purchase', {
                                value: total,
                                currency: 'USD',
                                contents: [{ id: 'unstuck_to_published', quantity: 1 }],
                            });
                        }
                    } else {
                        window.location.href = '/unstuck-to-published';
                    }
                    setIsLoaded(true);
                })
                .catch(() => {
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="max-w-4xl mx-auto px-6 py-20 flex flex-col items-center">
                ...
                <p className="font-lora text-xl text-gray-600 max-w-2xl text-center mb-12">
                    Success! You now have access to <strong>Unstuck to Published</strong> — the live workshop that turns your stuck ideas into published, revenue-generating Substack content.
                </p>
                ...
                <div className="w-full mb-8 bg-[#ffc300]/10 border-2 border-[#ffc300] p-8 rounded-2xl">
                    <div className="flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div className="flex-grow text-center md:text-left">
                            ...
                            <h2 className="font-anton text-2xl text-[#333333] uppercase mb-2">Unstuck to Published — Live Workshop</h2>
                ...
                <div className="grid md:grid-cols-2 gap-8 w-full mb-16 mt-6">
                    ...
                    <div className="bg-[#ffc300]/5 p-8 rounded-xl border border-[#ffc300]/20">
                        ...
                        <ul className="space-y-4">
                            {[
                                'Unstuck to Published — Live Workshop',
...
export default function UnstuckSuccessPage() {
    return (
