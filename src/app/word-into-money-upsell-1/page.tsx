...
import { AlertCircle, Shield, Lock, CheckCircle, ArrowRight } from 'lucide-react';
...
function PaymentForm({ clientSecret, leadId, wimLeadId }: { clientSecret: string; leadId: string; wimLeadId: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error, paymentIntent } = await stripe.confirmPayment({
            elements,
            confirmParams: { return_url: `${window.location.origin}/word-into-money-upsell-2?leadId=${wimLeadId}` },
            redirect: 'if_required',
        });

        if (error) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (window as any).__track?.checkoutStep?.('payment_error', 'wim-upsell-1-checkout');
            setErrorMessage(error.message || 'An error occurred.');
            setIsProcessing(false);
        } else if (paymentIntent?.status === 'succeeded') {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (window as any).__track?.formSuccess?.('wim-upsell-1-checkout');
            await fetch('/api/word-into-money/upsell-1/confirm-payment', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ leadId, paymentIntentId: paymentIntent.id, wimLeadId }),
            });
            window.location.href = `/word-into-money-upsell-2?leadId=${wimLeadId}`;
        }
    };

    return (
        <form onSubmit={handleSubmit} data-track-form="wim-upsell-1-checkout">
            ...
            <button disabled={isProcessing || !stripe} className={`w-full mt-6 bg-brand-lime hover:bg-brand-limeDim text-brand-950 font-anton text-xl py-5 rounded-xl uppercase ${isProcessing ? 'opacity-70' : ''}`}>
                {isProcessing ? 'Processing...' : 'GET FIRST 100 PAID SUBSCRIBERS — $97'}
            </button>
...
function Upsell1Content() {
    ...
    const startPayment = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const res = await fetch('/api/word-into-money/upsell-1/create-payment-intent', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (data.success) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (window as any).__track?.checkoutStep?.('payment_step', 'wim-upsell-1-checkout');
        ...
        <div className="min-h-screen bg-brand-950">
            <div className="max-w-[620px] mx-auto px-4 py-12">
                <div className="text-center mb-10">
                    <p className="text-brand-lime text-xs font-bold uppercase tracking-widest mb-3">Upsell 1 of 3 — One-Time Offer</p>
                    <h1 className="font-anton text-3xl md:text-5xl text-brand-white uppercase mb-4">Your First 100 Paid Subscribers</h1>
                    <p className="font-lora text-lg text-brand-grey max-w-md mx-auto">
                        You just unlocked the copywriting skills. Now here&apos;s how to convert those skills into your first 100 paying subscribers.
                    </p>
                </div>

                <div data-track-section="offer" className="space-y-4 mb-10">
                    {['The Paid Subscriber Engine framework', 'Upgrade email sequence templates', 'Objection-crushing copy vault', 'Tiny list monetisation playbook', 'Viral Substack Notes generator'].map((item, i) => (
                ...
                <div data-track-section="checkout" className="bg-brand-900 border border-brand-800 rounded-2xl overflow-hidden">
                    ...
                    <div className="p-6 md:p-8">
                        {step === 'info' ? (
                            <form onSubmit={startPayment} data-track-form="wim-upsell-1-checkout" className="space-y-4">
                                ...
                                <button disabled={isLoading} data-track="cta" data-track-id="wim-upsell-1-accept" className="w-full bg-brand-lime hover:bg-brand-limeDim text-brand-950 font-anton text-xl py-5 rounded-xl uppercase">
                                    {isLoading ? 'Preparing...' : 'CONTINUE TO PAYMENT — $97'}
                                </button>
                            </form>
                ...
                <div className="text-center mt-6">
                    <a href={`/word-into-money-upsell-2?leadId=${wimLeadId}`} className="text-brand-grey text-sm hover:text-brand-white transition-colors">
