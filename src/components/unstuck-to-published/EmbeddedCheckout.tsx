...
const CheckoutFormContent: React.FC<CheckoutFormProps> = ({ clientSecret, leadId, hasSdtBump, hasGeniusBump, hasHooksBump, hasBundle }) => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (error) {
            ...
            (window as any).__track?.checkoutStep?.('payment_error', 'unstuck-checkout');
            setErrorMessage(error.message || 'An unexpected error occurred.');
            setIsProcessing(false);
        } else if (paymentIntent && paymentIntent.status === 'succeeded') {
            // Conversion — payment succeeded.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (window as any).__track?.formSuccess?.('unstuck-checkout');
    ...
    const startCheckout = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (window as any).__track?.checkoutStep?.('payment_step', 'unstuck-checkout');
    ...
    return (
        <div id="checkout-section" className="w-full max-w-[660px] mx-auto bg-white rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden">
            ...
            <div className="p-8 md:p-12">
                {step === 1 ? (
                    <form onSubmit={startCheckout} data-track-form="unstuck-checkout" className="space-y-6">
                        <div>
