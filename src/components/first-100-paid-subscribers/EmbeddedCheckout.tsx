...
const CheckoutFormContent: React.FC<CheckoutFormProps> = ({ clientSecret, leadId }) => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const { error, paymentIntent } = await stripe.confirmPayment({
            elements,
            confirmParams: {
                return_url: `${window.location.origin}/10k-launch-lab-upsell?leadId=${leadId}&source=first100`,
            },
        ...
        if (error) {
            ...
            (window as any).__track?.checkoutStep?.('payment_error', 'first-100-checkout');
            setErrorMessage(error.message || 'An unexpected error occurred.');
            setIsProcessing(false);
        } else if (paymentIntent && paymentIntent.status === 'succeeded') {
            // Conversion — payment succeeded.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (window as any).__track?.formSuccess?.('first-100-checkout');
            ...
            window.location.href = `/10k-launch-lab-upsell?leadId=${leadId}&source=first100`;
        }
    };
    ...
    const startCheckout = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (window as any).__track?.checkoutStep?.('payment_step', 'first-100-checkout');
    ...
    return (
        <div id="waitlist-section" className="w-full max-w-[600px] mx-auto bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden">
            ...
            <div className="p-8 md:p-12">
                {step === 1 ? (
                    <form onSubmit={startCheckout} data-track-form="first-100-checkout" className="space-y-6">
                        <div>
